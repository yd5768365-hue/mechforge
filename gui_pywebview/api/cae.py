...
提供与 `mechforge_work` 的 CAEEngine 的桥接：
- 依赖与状态检查
- 内置悬臂梁 Demo（含理论值对比）
"""
...
class DemoResultResponse(BaseModel):
    ...
    max_displacement: float | None = Field(
        None, description="计算得到的最大位移 (mm)"
    )
    max_von_mises: float | None = Field(
        None, description="计算得到的最大 von Mises 应力 (MPa)"
    )
    theoretical_displacement: float = Field(
        0.0254, description="理论端部挠度 (mm)，基于 δ = FL³/(3EI)"
    )
    displacement_error: float | None = Field(
        None, description="绝对误差 |数值-理论| (mm)"
    )
    error_ratio: float | None = Field(
        None, description="相对误差 |数值-理论|/理论，单位 1"
    )
    num_nodes: int | None = Field(
        None, description="网格节点数（仅在依赖完整且求解成功时提供）"
    )
    num_elements: int | None = Field(
        None, description="网格单元数（仅在依赖完整且求解成功时提供）"
    )
