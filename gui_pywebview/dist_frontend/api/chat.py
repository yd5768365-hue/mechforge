...
async def _chat_with_gguf(user_message: str, context: str, rag_used: bool) -> ChatResponse:
    """使用 GGUF 后端进行聊天"""
...
async def _chat_with_ollama(user_message: str, context: str, rag_used: bool) -> ChatResponse:
    """使用 Ollama 后端进行聊天"""
    llm = get_llm_client()
    try:
        # 构建消息列表，包含系统提示词
