...
def build(onefile: bool = True, windowed: bool = True, optimize: bool = False) -> bool:
    ...
    # 收集 llama-cpp-python 动态库（包含 .dll/.so/.dylib）
    cmd.extend(["--collect-all", "llama_cpp"])
    print_info("添加 llama-cpp-python 动态库")

    # 收集 numpy（llama-cpp-python 依赖）
    cmd.extend(["--collect-all", "numpy"])
    print_info("添加 numpy 库")
